...
%
The teacher begins writing a function called \li{weighted_average} that will
...
(\ie{}~\li{a}) and values it produces are assigned corresponding unique dynamic
identifiers (\ie{}~\li{a.1}, \li{a.2}, etc{}).
%
...
grades.
%
The teacher builds the summand weighting the \li{hw1} score to account for 10\%
...
these six possible.
%
Initially, each of values in the \li{cutoffs} is a hole.
...
%
The teacher sorts the \li{weighted_averages} because she intends to look at the
data to help select the cutoff values, and then maps them to \li{letter_grades}.
...
\li{n >= cutoffs.a}, is indeterminate because \li{cutoffs.a} is.
%
...
\parasection{Example 5: \rkc{...}}

\rkc{gradual typing error, cast error, run around}
