...
%
%(Holes in \Hazelnut are \emph{unnamed}).
%
...
%
Rule {SVar} synthesizes the corresponding type from $\hGamma$. 
...
%
\Figref{fig:expansion} defines this expansion, and \Figref{fig:hasType} defines type assignment for internal expressions.
%
...
%
The following theorem establishes that expansions are well-typed and in the analytic case that the type $\htau'$ is consistent with $\htau$.
%
...
%
Intuitively, the subsumption rule permits us
to check an external expression against any type consistent with the
type that the expression \emph{actually} synthesizes:
%
