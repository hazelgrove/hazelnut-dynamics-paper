...
In this case, there are three instances of hole \li{1} in the result, numbered sequentially: \li{1:1}, \li{1:2} and \li{1:3}, arising from the three calls to \li{weighted_averages}. 
% An environment is a partial mapping of variables in scope to values. 
The programmer can select different closures by clicking on a hole instance in the result (the first instance, \li{1:1}, is selected by default, as indicated by the purple outline), or cycle through all of the closures for the hole at the cursor by clicking the arrows, e.g. $\blacktriangleright$, or using hotkeys.
